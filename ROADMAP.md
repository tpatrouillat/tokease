...
| **Lovable / Cursor / Codex / other AI tools** | Each tool has its own quota system. Most don't expose limits the way Claude Code's statusline does. Adding them = waiting on each vendor to publish (or reverse-engineering each one and inheriting N break-points). Checked in September 2026: only Codex writes its quota to a local file without a token, and it writes it into a conversation log rather than a quota file — too wide a read surface for what it buys, so it is not being built. It would be reconsidered if the Codex CLI ever persists a dedicated quota file. No integration is promised for any of these. |
