---
name: Bug report
about: Something doesn't work as expected
title: ''
labels: bug
assignees: ''
---

**What happened?**
<!-- What the menu bar showed vs. what you expected. Include the exact glyph if relevant: ⚙ … ? -->

**Environment**
- macOS version (`sw_vers`):
- Claude plan: Pro / Max  <!-- Free, Team, Enterprise are not supported -->
- Claude desktop app running? yes / no
- Claude Code version (`claude --version`), if you use the statusline:  <!-- needs ≥ 2.1.x -->
- "Updated" line shown in the dropdown (`Updated: HH:MM (via Claude app)`, `Updated: HH:MM (via Claude Code)`, or `⚠ HH:MM · stale 3h (Claude app idle?)`):
- Install method: Homebrew / from source
- Tokease version:

**`~/.tokease/usage.json`** (anonymise if needed)

```json

```

**`~/.tokease/statusline.err`** (last few lines, if it exists; redact paths, account details, and other sensitive data)

```text

```

**Steps to reproduce**
1.
2.
